...
def main():
    ...
    parser.add_argument('-s', '--size', metavar=_('WIDTHxHEIGHT'), help=_('Stage size in pixels'), type=str, default='1280x720')
    # 弹幕字体
    ...
    parser.add_argument('-fs', '--fontsize', metavar=_('SIZE'), help=(_('Default font size [default: %s]') % 25), type=float, default=75.0) # initial = 25.0
    # 弹幕不透明度
    ...
    # 保留底部多少高度的空白区域
    parser.add_argument('-p', '--protect', metavar=_('HEIGHT'), help=_('Reserve blank on the bottom of the stage'), type=int, default=3)
    # 当屏幕满时减少弹幕数
